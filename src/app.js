...
// parsing data & cookies terlebih dahulu
app.use(express.urlencoded({ extended: true }));
app.use(express.json());
app.use(cookieParser());

// opsional tapi disarankan (untuk sesi)
app.use(session({
...
// baru setelah itu set user dari cookies
...
app.get('/dashboard/freelancer', protect, (req, res) => {
  if (req.user.role !== 'freelancer') {
    return res.status(403).render('error', {
      title: 'Akses Ditolak',
      message: 'Hanya freelancer yang dapat mengakses dashboard ini.'
    });
  }
...
app.get('/dashboard/client', protect, (req, res) => {
  if (req.user.role !== 'client') {
    return res.status(403).render('error', {
      title: 'Akses Ditolak',
      message: 'Hanya klien yang dapat mengakses dashboard ini.'
    });
  }
...
app.get('/privacy-policy', (req, res) =>
  res.render('privacy-policy', { title: 'Kebijakan Privasi' })
);
app.get('/terms-of-service', (req, res) =>
  res.render('terms-of-service', { title: 'Ketentuan Layanan' })
);
app.get('/cookie-policy', (req, res) =>
  res.render('cookie-policy', { title: 'Kebijakan Cookie' })
);
...
if (process.env.NODE_ENV !== 'test') {
  bot.launch()
    .then(() => console.log('Bot Telegram sedang berjalan...'))
    .catch(err => console.error('Gagal menjalankan bot Telegram:', err));
}
